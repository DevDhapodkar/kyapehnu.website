...
export function ScrollytellingHero({ className = '' }: ScrollytellingHeroProps) {
  ...
  // Crossfade between primary model image and secondary fashion shopping image
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center px-4 md:px-12 pt-20 pb-8">
        ...
        <div className="absolute inset-0 pointer-events-none z-0">
          <div
            ...
            style={{
              background: isLight
                ? 'radial-gradient(circle, rgba(99,102,241,0.2) 0%, rgba(244,63,94,0.12) 50%, transparent 75%)'
                : 'radial-gradient(circle, rgba(255,255,255,0.3) 0%, rgba(120,80,220,0.15) 50%, transparent 75%)',
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto h-full grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-6 flex flex-col justify-center order-2 lg:order-1">
            <div className="relative w-full max-w-xl mx-auto lg:mx-0">
              ...
              >
                <p className={`text-xs tracking-[0.4em] font-mono mb-3 uppercase ${isLight ? 'text-neutral-500' : 'text-white/40'}`}>
                  01 / THE SOLUTION
                </p>
          ...
          {/* RIGHT COLUMN: Sleek Fashion Model / Shopping Image Display Card */}
          <div className="lg:col-span-6 flex justify-center order-1 lg:order-2">
            <motion.div
              className={`relative w-full max-w-sm sm:max-w-md aspect-[3/4] max-h-[50vh] sm:max-h-[60vh] lg:max-h-[70vh] rounded-3xl overflow-hidden glass-card shadow-2xl ${
                isLight ? 'border border-neutral-300/50' : 'border border-white/15'
              }`}
              style={{
                scale: modelScale,
            ...
            >
              {/* Primary High-Res Indian Fashion Model Image */}
              <div className="relative w-full h-full bg-neutral-900">
                ...
                {/* Secondary Fashion Shopping / Clothes Image on Scroll Crossfade */}
                <motion.div
                  className="absolute inset-0 z-10"
                  style={{ opacity: imageAltOpacity }}
                >
                  <Image
                    src="/images/hero-shopping-v1.jpg"
                    alt="Kya Pehnu Fashion Shopping Bags & Clothes"
                    fill
                    sizes="(max-width: 768px) 90vw, 45vw"
                    className="object-cover object-center"
                  />
              ...
              >
                <div className="px-3 py-1.5 rounded-full bg-black/80 backdrop-blur-md border border-white/20 text-white font-mono text-[10px] tracking-wider shadow-lg flex items-center gap-2">
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5 z-20 pointer-events-none">
          <div className={`w-px h-8 overflow-hidden relative ${isLight ? 'bg-neutral-400' : 'bg-white/20'}`}>
            <motion.div
              className={`absolute top-0 left-0 right-0 h-1/2 ${isLight ? 'bg-neutral-900' : 'bg-white/70'}`}
              animate={{ y: ['0%', '200%'] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
            />
          </div>
          <p className={`text-[9px] tracking-[0.25em] font-mono uppercase ${isLight ? 'text-neutral-500' : 'text-white/30'}`}>
            SCROLL
          </p>
        </div>
